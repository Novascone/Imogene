...
public partial class CustomSignals : Node
{
	...
	[Signal] public delegate void PlayerInfoEventHandler(Player s); // Send the player stats from the player to the UI
	[Signal] public delegate void AnimationFinishedEventHandler(string animation); // Send the player stats from the player to the UI
	[Signal] public delegate void AbilityAssignedEventHandler(string ability, string button_name, Texture2D icon); // Tells the player which ability has been assigned to which action button. Tells the HUD the same thing and send the icon of the ability as well.
	[Signal] public delegate void AbilityRemovedEventHandler(string ability, string button_removed); // Tells the player which ability has been assigned to which action button. Tells the HUD the same thing and send the icon of the ability as well.



	// Player to UI
	
	[Signal] public delegate void AvailableAbilitiesEventHandler(AbilityResource ability); // Tells the UI what abilities the player has available
	...
	[Signal] public delegate void SendStatsEventHandler(
												...
												int damage_bonus, float combined_damage, float base_aps, float aps_modifiers, float aps, float base_dps, float skill_mod,
												...
												int cold_resistance, int lightning_resistance, int holy_resistance, float maximum_health, float health_bonus, float health_regen, float health_regen_bonus, float maximum_posture,

												float posture_regen, float posture_regen_bonus, float health_on_retaliate, float resistance, float maximum_resource, float resource_regen, float resource_cost_reduction,
	...
	// Player to object
	[Signal] public delegate void TargetingEventHandler(bool targeting, Vector3 position); // Tells the targeting Icon to show or not show
	[Signal] public delegate void PlayerPositionEventHandler(Vector3 position); // Tells an object where the player is
	...
	[Signal] public delegate void AbilityAcceptEventHandler(); // Tells the UI that the ability assignment has been accepted
	[Signal] public delegate void AbilityCancelEventHandler(); // Tells the UI that the ability assignment has been accepted
	[Signal] public delegate void HideCursorEventHandler(); // Tells the UI to close cursor
	[Signal] public delegate void CurrentAbilityBoundOnCrossButtonEventHandler(Texture2D icon); // Tells the Ability category which button is currently bound
	[Signal] public delegate void SendConsumableIconEventHandler(Texture2D icon); // Tells the UI which icon to assign the consumable
	


	// Object to player
	[Signal] public delegate void EnemyPositionEventHandler(Vector3 position);  // Tells the player where the enemy is

	// UI to object
	[Signal] public delegate void ZoomCameraEventHandler(bool zoom);  // Tells the camera to zoom
