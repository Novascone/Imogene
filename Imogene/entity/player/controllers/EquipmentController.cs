...
public partial class EquipmentController : Node
{
	...
	public void AddEquipableStats(Player player, ArmsResource item)
	{
		...
		// player.base_stats["strength"] += item.strength;
		// player.base_stats["dexterity"] += item.dexterity;
		// player.base_stats["intellect"] += item.intellect;
		// player.base_stats["vitality"] += item.vitality;
